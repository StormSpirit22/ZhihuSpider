...
public class ContentParser {
	...
	public HashMap<String, String> parseZhiHu(String url, Long unixTime, int threadIndex){
        ...
        if(url.contains("http://www.zhihu.com/people/excited-vczh"))
        {
        	...
        	String content = sendRequest(url, "post");
        	if(content == null){
        		return null;
        	}
        	...
        	if(targetObject != null && moreContentUrl != null) {
        		result.put(targetObject, moreContentUrl);
        	}
...
//        	pageFetcher.sendPostToUrl(loginActionUrl, loginParameters);
	...
	public void analysisAnswer() {
		while(Initial.resultUrlsQueue.size() > 0) {
        	...
        	loginParameters.put("email", "XXXX");
        	loginParameters.put("password", "XXXX");
        	loginParameters.put("_xsrf", "68a22c4665852a3ed2353bd9a6877010");
        	loginParameters.put("remember_me", "true");
        	loginActionUrl = compoundUrlAndParameters(loginActionUrl, loginParameters);
        	sendRequest(loginActionUrl, "post");
	...
	public String compoundUrlAndParameters(String url, HashMap<String, String> parameters) {
